# AI Implementation Skill：ClinMindRuntime（Phase 8-P1 可实现但受限）
...
> Phase 8-P1 三份实现级文档已经建立，可以按任务清单进入 **ModelProvider / JudgeProvider / ProviderCapabilityProfile MVP** 的受限代码实现。
...
| 项 | 内容 |
|---|---|
| 当前阶段 | Phase 8-P1 可实现但受限 |
| 前置状态 | Phase 1–8 P0 已冻结 |
| 前置代码基线 | Phase 8-P0 commit `39f2435` |
| 当前实现目标 | JudgeProvider / RiskSignalClassifierProvider / ProviderCapabilityProfile MVP |

已完成主线：

- Phase 1-P0 ~ Phase 7-P1：已冻结
- Phase 8-P0：Python AI Provider / EmbeddingProvider MVP，已冻结
- Phase 8-P1：设计文档已建立，可受限实现
...
7. `docs/3-phase实现/Phase8_P0冻结记录.md`
...
1. 按 Phase8_P1 开发任务清单实现 JudgeProvider / RiskSignalClassifierProvider / ProviderCapabilityProfile MVP。
2. 扩展 Python provider：`/v1/judge`、`/v1/classify-risk`、`/v1/capability-profiles`。
3. 扩展 Java Provider domain：JudgeRequest、JudgeScoreResult、RiskSignalDraft、ProviderCapabilityProfile。
4. 新增 ProviderCapabilityPolicy。
5. 扩展 ProviderValidationService，校验 judge / risk / profile。
6. 扩展 Provider Debug API。
7. 新增 Judge / Risk / Profile Evaluation Scorer。
8. 新增 Trace / Audit 记录。
...
Phase 8-P0 只能被 Phase 8-P1 复用，不能继续向 P0 范围堆新能力。
...
```text
可以进入 Phase 8-P1 的 JudgeProvider / RiskSignalClassifierProvider / ProviderCapabilityProfile MVP 实现；
只能按三份 Phase 8-P1 文档受限实现；
不可以让模型能力接管 Runtime；
不可以让 Judge 直接决定诊断；
