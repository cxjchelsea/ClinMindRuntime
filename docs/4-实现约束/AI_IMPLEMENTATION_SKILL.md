...
| 项 | 内容 |
|---|---|
| 当前阶段 | Phase 11-P0 可实现但受限 |
| 前置状态 | Phase 1–10 P0 已冻结 |
| 前置冻结记录 | `docs/3-phase实现/Phase10_P0冻结记录.md` |
...
| 当前实现目标 | Role-based Frontend Suite MVP：Patient Portal / Clinician Workspace / Governance Console 三角色前端演示闭环 |
...
- Phase 11-P0：Role-based Frontend Suite 已完成设计，当前可进入受限实现

当前项目权威定位：

- ClinMindRuntime = 受控医疗 AI Agent Runtime 与能力治理平台
- Phase 11-P0 = 基于 RBAC 的多角色前端产品原型层
...
3. `docs/README.md`
4. `docs/3-phase实现/Phase11_P0RoleBasedFrontendSuite_实现规格.md`
5. `docs/3-phase实现/Phase11_P0FrontendRBAC_API与测试设计.md`
6. `docs/3-phase实现/Phase11_P0开发任务清单.md`
7. `docs/3-phase实现/Phase10_P0冻结记录.md`
8. `docs/3-phase实现/Phase10_P0人工测试结果.md`
9. `docs/3-phase实现/Phase10_P0GovernanceConsole_RuntimeConsole_实现规格.md`
10. `docs/3-phase实现/Phase10_P0Console_API与测试设计.md`
11. `docs/3-phase实现/Phase10_P0开发任务清单.md`
12. `docs/3-phase实现/Phase9_P0冻结记录.md`
13. `docs/3-phase实现/Phase9_P0人工测试结果.md`
14. `docs/3-phase实现/Phase8_P2冻结记录.md`
15. `docs/3-phase实现/Phase8_P1冻结记录.md`
16. `docs/3-phase实现/Phase8_P0冻结记录.md`
17. Phase 1–7 各阶段冻结记录

---

# 三、当前允许做的事情

当前只允许围绕 Phase 11-P0 任务清单进行受限实现：

1. 建立 RBAC role model 与 route matrix。
2. 实现 `RoleGuard`、`DemoRoleSwitcher`、`ForbiddenPage`。
3. 重构前端 Shell，使其支持 Patient / Clinician / Governance Portal。
4. 将 Phase 10-P0 Console 页面迁移或包装到 `/governance/*`。
5. 新增 Patient Portal 静态 / 半动态 MVP 页面。
6. 新增 Clinician Workspace 静态 / 半动态 MVP 页面。
7. 新增 role-specific DTO types 与 demo seed data。
8. 打通同一 `runtime-demo-001` 的三角色前端投影演示。
9. 编写前端 RBAC、RoleGuard、敏感字段禁显、页面渲染测试。
10. 运行并记录 `npm run typecheck`、`npm run build`；如执行 Vitest，必须如实记录结果和 caveat。
11. 对 Phase 1–10 已冻结阶段进行必要 bug fix、测试补强、文档修正，但不得借 bug fix 增加新能力。

---

# 四、当前禁止做的事情

1. 向 Phase 1–10 P0 已冻结阶段继续堆新能力（除非走 bug fix）。
2. 不做生产级登录、注册、身份认证系统。
3. 不接入真实患者数据。
...
Phase 6–10 P0 已冻结能力只能被 Phase 11-P0 按设计观察、复用和前端重组，不能继续向已冻结范围堆新能力。
...
不得跳过 RBAC 与 DTO 分层直接堆页面。

---

# 九、回归测试要求

Phase 11-P0 修改前端时，至少回归：
...
Phase 10-P0 Governance Console / Runtime Console MVP 已冻结；
Phase 11-P0 Role-based Frontend Suite 已完成设计，当前可进入受限实现；
允许实现 Patient Portal / Clinician Workspace / Governance Console 的多角色前端 MVP；
必须使用 RBAC route guard 与 role-specific DTO / mock projection；
