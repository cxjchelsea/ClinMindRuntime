# AI Implementation Skill：ClinMindRuntime（Phase 6-P0 可实现但受限）
...
> Phase 6-P0 三份实现级文档已经建立，可以按任务清单进入**受控 Agent 执行层 MVP**的受限代码实现。
...
```text
当前阶段：Phase 6-P0 可实现但受限
前置状态：Phase 1–5 已全部冻结
当前文档状态：总设计 v2.2 已完成，docs 已完成分层目录重构，Phase 6-P0 三份实现级文档已建立
当前实现目标：受控 Agent 执行层 MVP
```
...
Phase 6-P0：受控 Agent 执行层 MVP，可按文档受限实现
```
...
```text
1. 按 Phase6_P0开发任务清单实现受控 Agent 执行层 MVP。
2. 新增 Agent domain 基础对象。
3. 新增 AgentRegistry / AgentPolicy。
4. 新增 AgentRuntime。
5. 新增 InquiryPlanningAgent 的 rule-based / deterministic MVP。
6. 新增 InquiryPlanProposal / InquiryQuestionCandidate。
7. 新增 AgentProposalValidator / InquiryPlanProposalValidator。
8. 新增 RuntimeValidation 对 AgentProposal 的校验。
9. 新增 debug / internal Agent API。
10. 新增 AgentTrace / Audit 接入。
11. 新增 Agent Evaluation Scorer。
12. 已冻结阶段的 bug fix、测试补强、文档修正。
```
...
```text
1. 已实现哪些 Agent execution 对象。
2. InquiryPlanningAgent 的能力边界。
3. Agent 不能做什么。
4. API 与测试结果。
5. 人工测试结果。
6. 后置到 Phase 6-P1 / Phase 7 的任务。
```
...
```text
可以进入 Phase 6-P0 的受控 Agent 执行层 MVP 实现；
只能按三份 Phase 6-P0 文档受限实现；
不可以直接写自由 Agent；
