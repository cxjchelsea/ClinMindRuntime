...
RuntimeStatus：标记一次问诊处于创建、收集信息、触发安全门、等待医生、完成等状态。
RuntimeTrace：记录五个能力域如何影响本轮判断。
Short-term Context：保存短期对话历史，用于语言连续性，不作为诊断事实源。
CaseFrame：属于诊断状态域，保存结构化病例。
Knowledge Context：属于医学知识域，为本轮问诊提供路径、规则、证据和检查依据。
Experience Context：属于临床经验域，为本轮问诊提供相似经验和误诊提醒。
...
本章说明平台层如何支撑训练、治理、评估和管理。平台层不直接参与每一轮问诊推理，但它决定 Runtime 能力从哪里来、经验如何治理、输出权限如何授权、系统表现如何被评估。
...
第六章已经说明平台层如何训练、治理和评估系统。本章开始进入 Runtime 层。

Runtime 层负责一次问诊从输入到输出的完整运行过程。它不是一个固定 Workflow，而是一个状态驱动的诊断运行时：每一轮用户输入都会更新 RuntimeState，并由当前病例状态、医学知识、临床经验、危险信号、证据充分性和输出边界共同决定下一步动作。
...
本章说明如何把前面的能力域、平台层和 Runtime 层落到代码、数据表和交互协议中。
