...
本章专门说明共享能力资产层。共享能力资产层不是一次问诊中的执行流程，而是平台训练、治理、评估、审核和再认证之后沉淀出来的资产集合。

这些资产被 Runtime 执行层读取或检索，用来支撑一次问诊中的医学知识、临床经验、输出边界和复盘进化能力。

共享能力资产层的核心特点：

```text
1. 它不是临时上下文，而是可版本化、可审计、可更新的系统资产。
2. 它不直接生成最终回答，而是被 Runtime 执行层调用。
3. 它的更新必须经过训练、审核、评估或再认证流程。
4. 它是平台管理层和 Runtime 执行层之间的中间层。
```

## 7.1 Symptom Rotation Library：症状群训练库

Symptom Rotation Library 保存不同症状群的训练内容、标准病例、必问问题、高危排除点和评估标准。

它由 Training Center 管理，被 Runtime 通过 Knowledge Context 读取。
...
读取该症状群的常见候选诊断
读取该症状群的高危疾病和必问问题
为 Question / Test Policy 提供追问依据
为 Capability Profile 提供能力边界依据
```

## 7.2 Clinical Pathway：临床路径

Clinical Pathway 保存症状群或疾病方向上的标准临床处理路径。

它属于医学知识域，主要服务 SafetyGate、DDx Board、EvidenceGraph 和 Question / Test Policy。
...
## 7.3 KG-lite：轻量医学知识图谱

KG-lite 保存疾病、症状、危险信号、检查、风险因素之间的结构化关系。
...
```text
为候选诊断召回提供结构化关系
为 EvidenceGraph 提供证据连接
为 Question / Test Policy 提供下一步追问线索
为医生端解释提供结构化支撑
```

## 7.4 RAG Evidence Library：医学证据库

RAG Evidence Library 保存可检索的医学指南、文献、说明书、科普资料和内部知识文档。
...
```text
为 EvidenceGraph 提供可追溯依据
为医生端报告提供证据引用
为患者端科普表达提供安全来源
在 RAG 失败时触发 FailurePolicy 降级
```

## 7.5 Red Flag Rules：危险信号规则库

Red Flag Rules 保存各症状群下必须优先识别的危险信号规则。
...
```text
SafetyGate 读取危险信号规则
一旦命中高危规则，DecisionBoundary 收紧输出
高危候选不能因低置信度被删除
必要时进入急诊建议或转人工医生
```

## 7.6 Test Recommendation Rules：检查建议规则库

Test Recommendation Rules 保存不同症状群、候选诊断和证据缺口下的检查建议规则。
...
```text
Question / Test Policy 根据 EvidenceGraph 缺失证据读取检查建议
医生端显示检查目的
患者端只显示安全、必要且不过度诊断的检查准备或就医建议
```

## 7.7 Capability Profile：能力档案

Capability Profile 保存系统在不同症状群、不同运行模式下的能力等级和输出权限。

它由病例考试、评估结果和再认证记录共同更新，被 Runtime 通过 SafetyGate 和 DecisionBoundary 读取。
...
## 7.8 Clinical Experience Memory：临床经验记忆

Clinical Experience Memory 保存经过医生审核、质控和再认证的经验单元。
...
```text
Experience Context 检索相似经验
SafetyGate 根据经验提醒提高警觉
EvidenceGraph 记录经验提醒
Question / Test Policy 调整追问顺序
DecisionBoundary 在高风险经验触发时收紧输出
```
...
## 7.9 Experience Candidates：经验候选

Experience Candidates 是 Shadow Learning、医生反馈或病例复盘后产生的候选经验。

它还不是可被 Runtime 直接使用的经验资产。
...
医生审核状态
是否进入回归测试
是否转为 Clinical Experience Memory
```

流转：

```text
Shadow Learning / Doctor Feedback / Follow-up Outcome
  ↓
...
## 7.10 Evaluation Results：评估结果

Evaluation Results 保存病例考试、对照实验、回归测试和版本评估的结果。
...
```text
Evaluation & Recertification Center 读取评估结果
Capability Profile 根据评估结果升级或降级
Experience Memory Governance 根据回归结果决定经验是否上线
Audit & Governance Center 记录评估和发布过程
```

## 7.11 Recertification Records：再认证记录

Recertification Records 保存某个能力、经验、症状群或版本通过再认证的记录。
...
```text
Capability Profile 更新前必须读取再认证记录
DecisionBoundary 可根据认证状态限制输出
经验过期或认证失败时必须降权或下线
```

## 7.12 Audit Logs：审计记录

Audit Logs 保存系统关键动作、模型调用、知识检索、经验触发、安全判断、输出边界和版本发布记录。
...
```text
RuntimeTrace 写入关键判断过程
Audit & Governance Center 查看审计记录
复盘时根据审计记录定位错误来源
合规场景下提供可追踪证据
```
...
第六章已经说明平台管理层如何训练、治理和评估系统，第七章已经说明共享能力资产层如何承载平台形成的知识、经验和治理产物。本章开始进入 Runtime 执行层。
...
## 10.3 Knowledge Context：医学知识上下文
...
## 10.4 Experience Context：临床经验上下文
...
## 10.6 Differential Diagnosis Board：候选诊断状态板
...
## 10.8 Question / Test Policy：动态追问与检查建议
...
## 10.10 Human-like Interaction Layer：分角色表达
...
## 10.11 FailurePolicy：医疗 Fail-safe
...
第七章已经定义了 `Symptom Rotation Library`、`Clinical Pathway`、`Evaluation Results` 和 `Capability Profile` 等共享能力资产。本章不再重复定义这些资产，而是说明医学知识资产和能力档案如何通过训练、病例考试和评估被生成，并最终影响 Runtime 的输出权限。
...
系统为什么有资格处理某个症状群？
某个症状群的知识资产如何被构建？
病例考试如何产生 Evaluation Results？
Evaluation Results 如何更新 Capability Profile？
Capability Profile 如何限制 Runtime 的输出边界？
```

## 11.1 训练与授权总流程

训练与能力授权不是模型微调的同义词，而是平台对 Runtime 能力边界的生成和认证流程。
...
症状群选择
  ↓
构建 Symptom Rotation Library
  ↓
整理 Clinical Pathway / Red Flag Rules / Test Recommendation Rules
  ↓
构建标准病例与模拟患者
  ↓
病例考试
  ↓
生成 Evaluation Results
  ↓
评估危险信号识别、候选诊断、证据归因、输出边界
  ↓
更新 Capability Profile
  ↓
Runtime 根据 Capability Profile 控制输出权限
```
...
训练结果不能直接代表线上能力。
必须经过病例考试和评估。
能力授权必须细化到症状群、运行模式和输出等级。
能力不足时 Runtime 必须继续追问、收紧输出或转人工。
```

## 11.2 症状群定义与训练包构建

症状群是训练和授权的基本单位。系统不一开始承诺覆盖所有诊断问题，而是按症状群逐步训练、评估和授权。
...
病例考试用于评估 Runtime 是否真正掌握某个症状群的问诊和安全输出能力。

考试不只看最终诊断是否命中，而是看完整诊断行为：

```text
危险信号是否识别
高危疾病是否保留为 must_not_miss 或 need_to_rule_out
必问问题是否覆盖
候选诊断是否合理
证据归因是否正确
推荐检查是否合理
...
## 11.4 Evaluation Results 如何生成

病例考试结束后，平台生成 Evaluation Results。Evaluation Results 不是单一分数，而是一组面向 Runtime 行为的指标。
...
## 11.5 Capability Profile 如何更新

Capability Profile 不应由人工主观填写，而应由训练记录、病例考试、Evaluation Results 和再认证结果共同更新。
...
```text
如果高危漏放率超过阈值：不得升级，必要时降级。
如果输出越权率超过阈值：不得开放患者端诊断方向。
如果医生端摘要可用性不足：只能保留结构化证据展示。
如果某症状群病例覆盖不足：不得开放高等级能力。
如果新版本回归测试失败：保持旧能力档案或触发回滚。
```

Capability Profile 的更新结果会影响：
...
是否只能做风险提示
是否必须转医生
是否允许进入 Silent Evaluation
是否需要重新训练或再认证
```

## 11.6 能力授权如何影响 Runtime

Runtime 执行时不直接相信模型能力，而是读取 Capability Profile 作为输出边界依据。
...
如果证据不足，即使能力等级较高，也必须继续追问或建议检查。
如果评估或再认证过期，DecisionBoundary 应收紧输出。
```

因此，第十一章的本质是：

```text
训练和评估不是为了证明系统“很聪明”，而是为了生成可审计的能力边界。
Capability Profile 是训练结果进入 Runtime 的唯一授权入口。
```
...
第七章已经定义了 `Clinical Experience Memory`、`Experience Candidates`、`Evaluation Results` 和 `Recertification Records` 等共享能力资产。本章不再重复定义这些资产，而是说明临床经验如何从 RuntimeTrace、医生反馈和随访结局中产生，并经过审核、验证、上线、回滚和再认证后进入共享能力资产层。

本章回答的是：

```text
一次 Runtime 运行结束后，哪些信息可以进入复盘？
医生反馈和随访结局如何形成经验候选？
未经审核的经验为什么不能直接进入 Runtime？
经验如何通过审核、评估和再认证？
经验上线后如何影响 Runtime？
经验出错后如何下线或回滚？
```

## 12.1 经验来源：RuntimeTrace、医生反馈与随访结局

经验进化不能来自模型自我感觉，也不能来自单轮对话中的主观总结。它必须来自可追踪的运行记录和外部反馈。

主要来源：

```text
RuntimeTrace：记录本轮使用了哪些知识、经验、候选诊断和输出边界。
Doctor Feedback：医生是否采纳、驳回、修正 AI 的判断。
Final Diagnosis：最终诊断结果。
Follow-up Outcome：随访结局、病情变化、检查结果回填。
Audit Logs：关键安全判断、权限访问和版本记录。
```
...
## 12.2 Shadow Learning 如何生成 Experience Candidates

Shadow Learning 是后台经验发现机制，不直接改变线上 Runtime 行为。
...
输出不是 Clinical Experience Memory，而是 Experience Candidates。

```text
Shadow Learning 只能生成候选经验。
候选经验不能直接影响线上 Runtime。
候选经验必须经过医生审核和离线评估。
```

## 12.3 Experience Candidates 如何被审核

Experience Candidates 进入 Experience Memory Governance 后，需要经过审核和验证。
...
## 12.4 Clinical Experience Memory 如何上线、下线和回滚

只有通过审核和离线评估的经验，才能进入 Clinical Experience Memory。
...
## 12.5 Review & Recertification 如何更新经验和能力

Review & Recertification 是经验进入长期系统能力前的最后一道关口。
...
真实病例 / Silent Evaluation
  ↓
RuntimeTrace + Doctor Feedback + Follow-up Outcome
  ↓
错误归因
  ↓
Shadow Learning 发现 Experience Candidates
  ↓
医生审核
  ↓
离线评估和回归测试
...
## 12.6 Clinical Data Maturity Pipeline 如何保证数据分层

Clinical Data Maturity Pipeline 负责区分不同成熟度的数据，避免训练、评估、经验和真实病例相互污染。
...
医生反馈不能未经审核直接改变线上行为。
随访结局只能通过复盘和再认证影响能力资产。
```

因此，第十二章的本质是：

```text
经验进化不是在线学习，而是受控的临床经验治理流程。
Clinical Experience Memory 是经验治理流程的结果，不是模型自动记忆。
```
