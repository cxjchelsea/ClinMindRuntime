...
医学知识域不等于临床经验域。医学知识偏标准规则和可引用依据，临床经验偏真实场景中的误诊教训、相似病例和医生点评。
...
DDx Board 候选诊断状态
EvidenceGraph 证据状态图
Question / Test Policy 下一步动作
每轮状态更新日志
```

这个能力域决定系统是否真的从“聊天问答”升级为“诊断 Runtime”。
...
输出边界域是医疗 AI 项目的安全核心。它保证系统不是先生成医学回答再加免责声明，而是先判断当前是否允许输出，再生成表达。

## 2.6 复盘进化域

复盘进化域指系统如何从真实运行、医生反馈、最终诊断、随访结局和评估结果中成长。
...
复盘进化域让系统从一次性 Demo 变成可持续进化的平台。

## 2.7 能力域与模块的关系

五个能力域不是五个独立服务，而是系统能力的上层抽象。平台层和 Runtime 层中的具体模块，是这些能力域的工程落地方式。

一个能力域通常由多个模块共同支撑，一个模块也可能同时服务多个能力域。

| 能力域 | 主要模块 | 被哪些模块调用或影响 |
|---|---|---|
| 医学知识域 | Clinical Pathway、KG-lite、RAG Evidence Library、Red Flag Rules | SafetyGate、DDx Board、EvidenceGraph、Question/Test Policy |
| 临床经验域 | Clinical Experience Memory、Shadow Learning、Experience Governance | SafetyGate、EvidenceGraph、Question/Test Policy、DecisionBoundary |
...
实现时不需要强行创建五个大服务，而应在数据结构、运行流程、接口、目录、日志和评估中体现五个能力域。

RuntimeState 中可以这样承载五个能力域：
...
```text
knowledge_context = 医学知识域
experience_context = 临床经验域
diagnostic_state = 诊断状态域
boundary_state = 输出边界域
evolution_trace = 复盘进化域
```
...
用户输入
  ↓
Update CaseFrame：诊断状态域开始工作
  ↓
查询医学知识：医学知识域提供路径、规则、候选疾病、检查依据
  ↓
检索临床经验：临床经验域提供相似病例、误诊教训、must_ask
  ↓
SafetyGate：输出边界域判断是否高危
  ↓
DDx Board + EvidenceGraph：诊断状态域更新候选诊断和证据关系
  ↓
Question / Test Policy：诊断状态域决定下一步追问或检查建议
  ↓
DecisionBoundary：输出边界域决定患者端 / 医生端能输出什么
  ↓
Human-like Interaction Layer：生成安全表达
  ↓
RuntimeTrace：记录五个能力域各自如何影响本轮判断
  ↓
医生反馈 / 随访结局：复盘进化域更新经验候选和能力档案
```
...
ClinMindRuntime 由平台层和 Runtime 层共同组成。

平台层负责训练、治理、评估和管理；Runtime 层负责一次问诊过程中的状态维护、证据推理和安全输出。
...
平台模块不是五个能力域的简单拆分，而是围绕五个能力域提供管理、治理和评估能力。

```text
Training Center：主要支撑医学知识域、输出边界域和复盘进化域
Runtime Console：主要支撑诊断状态域、输出边界域和审计展示
Experience Memory Center：主要支撑临床经验域和复盘进化域
Evaluation & Recertification Center：主要支撑复盘进化域和输出边界域
Audit & Governance Center：横跨五个能力域，负责审计、合规和回滚
Role & Permission：主要支撑输出边界域，也约束平台层操作权限
```

## 5.3 能力域与 Runtime 模块的关系

Runtime 模块是五个能力域在一次问诊过程中的具体执行组件。

```text
Runtime API：承载一次问诊生命周期，调度五个能力域协同工作。
EntryAssessment：决定进入健康管理态、临床问诊态、急症提示或不支持场景。
RuntimeState：承载五个能力域在一次问诊中的状态快照。
Symptom Rotation Library：属于医学知识域和训练体系，为症状群提供路径、病例和评估标准。
Capability Profile：属于输出边界域，也被复盘进化域持续更新。
CaseFrame：属于诊断状态域，保存结构化病例。
SafetyGate：属于输出边界域，但依赖医学知识域和临床经验域。
Differential Diagnosis Board：属于诊断状态域，维护候选诊断状态。
EvidenceGraph：属于诊断状态域，同时引用医学知识和临床经验。
Question / Test Policy：属于诊断状态域，同时调用医学知识、临床经验和输出边界。
DecisionBoundary：属于输出边界域，决定患者端和医生端能输出什么。
Human-like Interaction Layer：属于表达层，不单独构成核心能力域，但必须受输出边界控制。
Clinical Experience Memory：属于临床经验域。
Shadow Learning：属于临床经验域和复盘进化域之间的经验候选发现机制。
Review & Recertification：属于复盘进化域。
Evaluation System：属于复盘进化域，同时反向影响 Capability Profile。
FailurePolicy：属于输出边界域，负责异常时 fail-safe。
```
...
识别症状群
  ↓
查询医学知识：Clinical Pathway / KG-lite / RAG Evidence / Red Flag Rules
  ↓
...
医生培养机制不是文档中的一个孤立章节，而是系统运行权限和经验进化的基础。
...
# 二十一、Shadow Learning 与经验治理

Shadow Learning 允许系统在后台发现病例模式，但不允许直接改变线上行为。
...
# 二十四、Review & Recertification 复盘与再认证
...
# 二十五、FailurePolicy 医疗 Fail-safe 策略
...
> 我的系统不是简单按照功能模块堆起来的，而是先抽象出五个核心能力域：医学知识、临床经验、诊断状态、输出边界和复盘进化。这五个能力域共同构成智能诊断的核心能力。具体实现时，再通过 Clinical Pathway、Experience Memory、CaseFrame、EvidenceGraph、SafetyGate、DecisionBoundary、Evaluation、Recertification 等模块落地。能力域是设计逻辑，模块是工程实现。
