...
Phase 4-P0：经验候选与训练数据候选沉淀机制已完成代码实现、人工 API 验收和冻结记录。
当前阶段：Phase 4-P0 freeze complete / Phase 4-P1 planning pending。
```

当前不应继续向 Phase 4-P0 堆新能力；后续改动限于 bug fix、测试补强、文档同步与第六节 hardening 评估。

当前最优先任务：

```text
1. 读取 docs/Phase4_P0冻结记录.md 了解冻结边界与 hardening backlog。
2. 进入 Phase 4-P1 前，先写 P1 详细设计与任务清单。
3. 接真实 RuntimeTrace 前，优先评估 training candidate 脱敏层。
4. 继续禁止提前接入 RAG、Python Provider、数据库、前端和模型训练。
```
...
7. docs/Phase4_经验候选与训练数据候选沉淀_实现规格.md
...
阅读目标：理解 Runtime、Provider、Evaluation、Safety、Trace、模型训练、AI 前沿技术之间的关系。
...
| `docs/Phase4_开发任务清单.md` | 已完成 / freeze 依据 | Phase4-P0-A 到 P0-G 完成情况 |
...
| `docs/Phase4_人工测试API结果.md` | 验收归档 | Phase 4 人工 API 验收记录 |
...
```text
1. 以 docs/Phase4_P0冻结记录.md 作为 Phase 4-P0 边界依据。
2. 进入 Phase 4-P1 前，先写 P1 详细设计与任务清单。
3. 接真实 RuntimeTrace 前，优先评估 training candidate 脱敏层。
4. 保持 Phase1/2/3/4 回归测试通过。
```
...
当前文档体系已支撑 Phase4-P0 实现并完成冻结。Phase3-P0 与 Phase4-P0 均已冻结，下一步是 Phase 4-P1 规划，而非继续向 P0 堆功能。